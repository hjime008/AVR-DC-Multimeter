...
void TimerOn() {
	...
	//TIMSK = 0x10; // bit1: OCIE1A -- enables compare match interrupt (for 16A)
...
void sm_tick()
{
	...
	switch (state) //Actions
	{
		...
		//{
			...
			//_delay_ms(5);
		//}
		//currResistance = currResistance;//take average of sum of readings
		
		//currResistance = currResistance + 10;
		...
		//for(unsigned char i = 0; i < 30; i++)//collect and sum 30 readings for averaging
		//{
			//currCurrent = currCurrent + ADCOutput(2);
			////_delay_ms(5);
		//}
		//currCurrent = currCurrent/30;//take average of sum of readings
		//rawCurrent = (uint16_t) currCurrent;
		
		//currCurrent = ((ADCOutput(2)-505)/0.0205)/1000000; //Scale up voltage. *5 for the voltage division factor, and another *5 to scale to reference voltage
		currCurrent = (ADCOutput(2));
		//currentDifference = abs(rawCurrent-508);
		//if(currCurrent < 2)
		//{
		   //currCurrent = 0;
		//}
